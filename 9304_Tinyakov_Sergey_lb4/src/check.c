...
int main(int argc, char **argv){
    ...
    Matrix *matrix1 = generate_matrix(height, width), *matrix2 = generate_matrix(width, height),
           ...
           *matrix_strassen = create_matrix(height, height, 0);
    if(!matrix1 || !matrix2 || !matrix_elem || !matrix_blocks){
        ret = 1;
        goto main_end;
    }
    //fill_matrix_with_values(matrix_elem, 0);
    //fill_matrix_with_values(matrix_blocks, 0);
    ...
    //strassen_multiplication(matrix1, matrix2, matrix_strassen, BLOCK_SIZE);
    strassen_pmm(matrix1, matrix2, matrix_strassen, threads_number, BLOCK_SIZE);
    for(long i = 0; i < height * height; i++){
        if(matrix_elem->data[i] != matrix_blocks->data[i] || matrix_elem->data[i] != matrix_strassen->data[i]){
            ret = 2;
